...
impl <Theme: dialoguer::theme::Theme> Prompter<Theme> {
	pub fn select_with_return<Prompt: Into<String>>(&self, prompt: Prompt, options: &Options<'_, DoneWithReturn>) -> Result<SelectionReturn> {
	...
	pub fn select<Prompt: Into<String>>(&self, prompt: Prompt, options: &Options<'_, Done>) -> Result<Selection> {
		let mut select = self.get_select(prompt);
